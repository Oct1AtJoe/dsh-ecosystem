...
function apply(ctx = {}) {
  ...
  const loadAssets = async (attempt = 1) => {
    try {
      ...
      try {
        ...
        if (cfgRes.ok) {
          const cfg2 = await cfgRes.json();
          settingsCharacter = cfg2?.config?.character ?? null;
        }
      } catch {
      }
      const pref = (() => {
        try {
          return localStorage.getItem("kanye-pet:character") ?? null;
        } catch {
          return null;
        }
      })();
      const roles = parseCharacters(manifest);
      const nextId = settingsCharacter && settingsCharacter in roles.characters ? settingsCharacter : pref !== null && pref in roles.characters ? pref : roles.defaultId;
      characterId = nextId;
  ...
  const switchCharacter = async (id) => {
    ...
    try {
      ...
      try {
        localStorage.setItem("kanye-pet:character", id);
      } catch {
      }
      transient = null;
      transientUntil = 0;
