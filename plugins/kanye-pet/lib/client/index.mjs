...
export function apply(ctx = {}) {
  // 浏览器 client half 已禁用：桌面宠物由 Tauri 独立窗口负责（desktop-tauri/），
  // 页面内不再渲染浮宠。host half 继续正常服务资产/状态/配置。
  return () => {}
  ...
  const loadAssets = async (attempt = 1) => {
    try {
      ...
      // 角色解析：DSH settings.character 优先 → localStorage 偏好 → manifest.default。
      // settings 由"可配置插件"卡片写入，跨设备同步；localStorage 仅本机兜底。
      let settingsCharacter = null
      try {
        const cfgRes = await fetch(`${ASSETS_URL.replace('/assets', '')}/config`)
        if (cfgRes.ok) {
          const cfg = await cfgRes.json()
          settingsCharacter = cfg?.config?.character ?? null
        }
      } catch {
        // config 不可达：保持 localStorage/默认回退
      }
      const pref = (() => { try { return localStorage.getItem('kanye-pet:character') ?? null } catch { return null } })()
      const roles = parseCharacters(manifest)
      const nextId = settingsCharacter && settingsCharacter in roles.characters
        ? settingsCharacter
        : pref !== null && pref in roles.characters ? pref : roles.defaultId
      characterId = nextId
  ...
  const switchCharacter = async (id) => {
    ...
    try {
      ...
      try { localStorage.setItem('kanye-pet:character', id) } catch { /* 隐私模式忽略 */ }
      transient = null
      transientUntil = 0
      joyUntil = 0
  ...
  // 派发方式：window.dispatchEvent(new CustomEvent('kanye-pet:say', { detail: { text } }))
  ...
  document.addEventListener('kanye-pet:say', onPetSay)
  document.addEventListener('kanye-pet:fx', onPetFx)
  document.addEventListener('kanye-pet:status', onPetStatus)
  ...
  const dispose = () => {
    ...
    document.removeEventListener('kanye-pet:say', onPetSay)
    document.removeEventListener('kanye-pet:fx', onPetFx)
    document.removeEventListener('kanye-pet:status', onPetStatus)
