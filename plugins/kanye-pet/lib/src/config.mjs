...
export const NAMESPACE = 'kanye-pet'
...
export const DEFAULTS = Object.freeze({
  ...
  // 通知音效（只作用于 Tauri 桌宠窗口的 playNotifSound；Web GUI 浮宠不播音效）
  sound: {
    enabled: true,        // 音效总开关；false = 完全静音（气泡照常弹，只是不出声）
    volume: 0.3,          // 音量 0–1，自定义 wav 与合成音共用同一基准
  },
})
...
export function buildSchema() {
  return z.object({
    ...
    walk: z.object({
      ...
      sound: z.object({
        enabled: z.boolean().default(DEFAULTS.sound.enabled),
        volume: z.number().min(0).max(1).default(DEFAULTS.sound.volume),
      }),
    })
}
