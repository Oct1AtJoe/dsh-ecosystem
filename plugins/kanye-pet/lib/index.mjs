...
import { sanitizeAssetPath, contentTypeFor, cacheControlFor, ASSETS_PATH } from './src/assets.mjs'
...
const STATE_FILE = join(DSH_HOME, 'data', 'kanye-pet', 'state.json')
...
function json(res, status, body, extra = {}) {
  res.writeHead(status, {
    ...
    // 桌宠窗口（Tauri WebView2）从 http://tauri.localhost 跨源请求，需 CORS *
...
export function apply(ctx) {
  ...
  const resolveSessionTitle = (s) => {
    // rc.1 Session 的日志已私有化，公开读法是 snapshotEvents()（session.events 恒 undefined）
    const fromLog = titleFromLog(typeof s?.snapshotEvents === 'function' ? s.snapshotEvents() : [])
  ...
  const known = sessionViews.get(id)
        ...
        if (configRef.desktopPetEnabled !== false) {
          ...
          if (!lastTurnEndNotif.has(id)) {
            ...
            // 无条件记录（含 -1）：否则新会话首个 turn/end 会重进 seed 分支被当历史回合抑制
            ...
            if (petNotification === null && configRef.desktopPetEnabled !== false
              ...
              try {
                for (const s of sessionsSvc.list()) {
                  ...
                  const events = typeof s.snapshotEvents === 'function' ? s.snapshotEvents() : []
                  ...
                  if (lastTurnEnd !== null) {
                    ...
                    if ((lastTurnEnd.seq ?? 0) > lastNotifSeq) {
                      ...
                      setPetNotification(sid, reason, s, 'poll')
                      lastTurnEndNotif.set(sid, lastTurnEnd.seq)
                      console.log(`[kanye-pet] /state poll fallback: notified ${sid} reason=${reason}`)
                    }
                  }
                }
              } catch {
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            // 缓存策略单一来源 src/assets.mjs 的 cacheControlFor：
            // manifest.json 与 sounds/ 走 no-cache（manifest 是客户端首个请求且清单会变；
            // sounds/ 是用户可替换的通知音效，下发 immutable 会让「放好文件重启即可」静默失效）。
            // 其余随包素材（characters/…）immutable 一年：改素材必须改文件名或角色 id。
            res.writeHead(200, { 'content-type': contentTypeFor(rel), 'cache-control': cacheControlFor(rel), 'access-control-allow-origin': '*' })
            res.end(data)
          } catch {
  ...
  webServer.register({
    ...
    return () => {
      clearTimeout(saveTimer)
      saveState(state) // 末次落盘：disable/卸载前保留最终状
