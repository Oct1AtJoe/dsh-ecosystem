 ...
 */
function mutationHunks(name: string, argsRaw: string): readonly ProducedHunk[] {
  let args: unknown
  try {
    args = JSON.parse(argsRaw) as unknown
  } catch {
 ...
 */
export function stripContext(oldText: string | null, newText: string): ProducedHunk {
  if (oldText === null) return { oldText: null, newText }
...
function sideLinesRaw(text: string): string[] {
  if (text === '') return []
...
export const deliverablesDefinition: ConversationNodeDefinition<DeliverablesState> = {
  ...
  start: (_context, match, reader) => {
    ...
    return {
      ...
      calls: new Map<string, CallMutation>(),
      produced: [],
  ...
  update: (context, match) => {
    ...
    const additions: ProducedPath[] = [{ seq: match.event.seq, path }]
