# @deepseek-ai/dsh-client-ui-deliverables-custom
...
`ProducedFiles` renders the row between the closing message's body and its IconActions footer: a quiet label and one measured file lane. It shows the largest leading prefix that fits (up to six chips; basename text, full path as the `title`) while reserving the exact localized `+ N files` width, so the remainder stays visible without wrapping or horizontal scrolling. Each chip opens through the owner-supplied `openFile` — the same Host opener the tool rows use, with the chat view resolving relative paths against the session cwd. When files are hidden, a second-line **Show in folder** action opens the session workspace folder in the Host desktop's file manager (`remote.session.openWorkspacePath` with default open semantics, not `reveal` — on Windows `reveal` is `explorer /select`, which opens a directory's PARENT) — a folder is no resource address the Sidebar can claim, so it does not go through `openFile`; it appears only while the page is loopback and the current Host handshake reports `canOpenPath`, so direct remote Web and headless/container Linux Hosts omit the action by default. Design rationale: the [workspace file links Agent Note](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.md).

Each Turn's published `DeliverablesTurnData` also carries a conversation-cumulative change history: `deliverablesDefinition` chains the previous Turn's history through the start reader and appends the APPLIED hunks of every successful mutation (the `tool/result` diff view first, the call view's intended change as fallback) per path. A chip whose file has history shows the `+added -removed` line totals next to its name (the same full-side counting the diff primitive draws) and a chevron that expands the change below the row into one card: a title bar — the path (name plus dimmed directory), the same totals, and a collapse control of its own — over the diff primitive's body, with the primitive's path headers and footer turned off. A file written in an earlier Turn and edited again shows the whole conversation's edits on its chip. A compaction that drops the earlier Turn restarts the history; generic-edit cards carry no hunks and stay plain chips.
...
- **Bundled `DiffBlock` fork.** The expanded change panel needs the `showPathHeaders`/`showFooter` chrome toggles, so this package carries its own copy of `src/client/DiffBlock.tsx` (plus its module CSS and spec) while the official `ui-primitives` DiffBlock stays upstream. Upstream DiffBlock fixes must be re-applied to the copy.
