...
### Fixed
- **Git 提交流程暂存区隔离缺陷（部分暂存时未暂存文件被一并强制提交）**：
  - 根因定位：原先在 `src/host/git-service.ts` 的 `commit()` 方法中，粗暴执行了 `git add -A`，
    把工作区里所有未暂存的修改和未跟踪文件一网打尽强行加入了暂存区后再执行 `git commit`；
    导致用户在界面上手动精细挑选暂存部分文件后，点击提交时未暂存的文件依然被强制打包入库，暂存区边界彻底失效；
  - 彻底修复：
    1. 宿主侧彻底移除 `commit()` 中的 `['add', '-A']` 动作，严格还原 Git 原生暂存区隔离契约：
       仅提交已暂存（staged）文件，未暂存（unstaged）文件 100% 完好保留在工作区；
    2. 当暂存区为空时，Git 原生会拒绝提交；后端将此情况识别为明确的 `empty-stage` 错误码；
    3. 前端在回车提交与点击提交按钮入口处增加双重守卫，暂存区为空时直接给出醒目的红色错误提示
       “暂存区为空，无法提交：请先暂存要提交的文件”，绝不盲目提交；
    4. 同步更新三语 README 描述，消除过时的 `git add -A` 行为说明；
  - 新增 `scripts/test-commit-staged.ts` 专项测试套件（24 项断言），覆盖“部分暂存提交、未暂存保留工作区、空暂存拒绝”全流程。
