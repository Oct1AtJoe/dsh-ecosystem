...
- **图谱分支列被挤出默认视口、提交文本截断缺少完整内容查看途径**：
  - 根因：原先分支列写死 `BRANCH_COL_WIDTH = 240`，且提交列宽度 `autoCommit` 贪婪膨胀至
    全量提交中最长条目的像素宽度，两者相加导致默认图谱宽度 `graphWidth` 达 600px~800px+，
    而分支列绝对定位在最右侧，在默认宽度约 300px 的侧边栏视口下完全被挤出屏幕盲区；同时被截断文本缺少悬停展示；
  - 现将分支列调整为紧凑合理的 96px，未拖拽时提交列自动弹性填满视口剩余空间，
    使线图、提交信息与分支列**默认 100% 完整可见，无横向滚动条**；侧边栏拖宽时提交列自动响应式展宽；
    手动拖拽分隔线仍可自定义列宽并持久化；
  - 为提交节点 `<g>` 与分支文本标签注入原生 SVG `<title>` 悬停提示，鼠标 hover 即可即时查看完整
    提交主题、SHA、作者与提交时间；点击节点时底部详情卡片支持多行完整换行展示。

- **`AU` / `UA` / `DD` 三种冲突码同时漏进两个分组**（上游既有缺陷，非本次引入）：
  - `classifyChanges()` 只认 4 个冲突码（`UU` `AA` `UD` `DU`），但 git 的冲突码共 **7 个**；
    `statusLetter()` 早已把 `AU` / `UA` / `DD` 也判为 `C`，两处口径不一致；
  - 后果（实测复现）：同一个未解决的冲突文件**同时出现在「已暂存的更改」和「更改」两组**，
    而「冲突文件」组里反而没有它——用户看不到提示，甚至能对它点批量暂存，
    把没解决的冲突塞进 index；
  - 现统一收敛到唯一真相源 `statusLetter(code) === 'C'`，不再维护第二份码表。
    7 个码混排的回归自检已证明：回退成 4 码写法时 4 项断言失败且输出直接复现泄漏，修复后 32/32 通过。

- **「储藏改动」没有把未跟踪文件收进抽屉**：
  - 根因：`stashPush()` 用的是裸 `git stash push`。实测确认它的收纳范围是
    **已暂存的改动 + 已跟踪文件的未暂存改动**，但**绝不碰未跟踪文件**——
    于是点完「储藏改动」，新建的文件仍挂在变更列表里，用户以为没生效；
  - 现改为 `git stash push -u`（`--include-untracked`）。实测：已暂存 / 未暂存已跟踪 /
    未跟踪三类一共 5 个文件全部收走，工作区回到干净状态；`pop` 后 5 个又全部回来。

- **状态行显示的文件数量文案冗余，改为显示抽屉内容**：
  - 原先状态行显示 `N 个文件已改动 · A 已暂存 · B 未暂存`，但这是**冗余信息**——
    下方变更列表已按「已暂存的更改 / 更改」两组把每个文件逐条列出，再报一遍数字
    没有增量；而抽屉（stash 栈）里的内容**不在变更列表里**，用户无从判断
    「到底储藏进去没有、里面有几个文件」；
  - 现改为显示 **`已储藏 N 个文件`**，抽屉为空时不显示任何文案。新增宿主路由
    `POST /git-panel/stash-summary` 提供条目数与文件总数。
  - 统计时踩到并规避了一个坑：`git stash show --name-only` **默认不列出未跟踪文件**
    （`-u` 产生的储藏把未跟踪部分挂在第三个 parent 上），必须显式加
    `--include-untracked`，否则「储藏了 5 个文件」会被算成 2 个——数字直接骗人。
  - 顺带移除因此失效的 `statusText` / `statusLoaded` 状态与 `.dsh-gp-write-stash` 样式
    （后者是上游遗留的死代码：`setStashText` 从未被调用）。
...
### Changed
- **提交图谱改为时间降序排列（最新提交在顶部，历史向下滑动）**：
  - 原先在 `layoutGraph()` 实现中错误使用了 `row = commits.length - 1 - index`，
    强行将最新提交（index 0）映射到底部最大行号，最旧的初始化提交反而占了顶部的 `row: 0`；
    用户打开图谱第一眼看到的是远古提交，想看最新提交必须滚到最底端；
  - 现修正为 `row = index`，恢复接口设计契约（`row: 0 = 最新/顶部`），与 GitLens、
    Sourcetree、VS Code 等主流 Git 客户端习惯完全一致；
  - 贝塞尔曲线连线的控制点公式 `dy = Math.min(12, (py - y) / 2)` 正向契合从子提交（上方 y）
    垂直向下平滑弯曲进入父提交（下方 py）的流向；各分支 tip 胶囊标签自然对齐在顶部各分支最新提交旁；
  - 新增 `scripts/test-graph.ts`（11 项），严密覆盖单链历史、分叉合并 DAG、父子垂直拓扑约束与边界情况。

- **变更列表行首改用语义字母徽章，未跟踪与已修改一眼可分**：
  - 原先三组行首一律显示 porcelain 原始码（`??` / ` M` / `UU`），其中 ` M` 的**前导空格
    在界面上完全不可见**，`??` 也不说明「这是新文件、且尚未纳入 git」——用户无法直观
    分辨哪些是已更改、哪些是未跟踪；
  - 现改为单个语义字母 + 配色，**词汇复用文件树装饰已有的 `statusLetter()`**，
    因此同一个文件在面板与右侧「文件」树里是同一个字母（不引入第二套说法）：
    `U` 未跟踪 / `M` 已修改 / `A` 新增 / `D` 删除 / `R` 重命名 / `C` 冲突；
    悬停给出本地化解释，冲突类附原始码（`UD` / `UA` / `DD` 决定先取哪一侧，单靠 `C` 会丢信息）；
  - **未跟踪 ≠ 新增**：`??` 不暂存就不会进提交，`A` 表示已在 index，两者词汇与配色分开表达；
  - 三组（冲突 / 已暂存 / 更改）**词汇统一**，不再出现两组字母、一组原始码的割裂；
  - 配色走 `--dsh-gp-*` 主题令牌而非文件树那组 hex：`STATUS_COLORS` 是 VS Code **深色**主题色，
    实测白底对比度 `M` 1.73:1、`U` 2.00:1、`C` 3.20:1，**均不达 WCAG AA**；
    现方案深浅两色全部过 AA（4.87:1–8.12:1），以 computed style 探针与 PNG 像素解码双重核验。
  - 顺带移除上一版遗留的死测试：`test-status-summary.ts` 仍在复刻 Panel 已删除的
    `statusSummary` 算法（该位置现显示抽屉摘要），共 5 项断言验证的是不存在的代码。
