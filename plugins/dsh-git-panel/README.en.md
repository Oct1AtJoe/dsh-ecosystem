...
- **Write bar** (top of the panel, under the tabs):
  - **Commit**: type a message and press Enter → commit **staged** files only (`git commit -m`). Strictly isolates the index: unstaged files are never committed and stay in the working tree. Refuses to commit with a clear notice when nothing is staged
...
- **Changes + syntax-colored Diff**: uncommitted changed files are listed under the write bar with a **semantic letter badge** (`U` untracked / `M` modified / `A` added / `D` deleted / `R` renamed / `C` conflict — the same letters as the file-tree badges, hover for the plain-language meaning, so new untracked files are immediately distinguishable from edited ones); click any file to view its full diff against HEAD with line-by-line coloring (green + for additions, red - for deletions, blue @@ for hunks) — review changes clearly before committing
