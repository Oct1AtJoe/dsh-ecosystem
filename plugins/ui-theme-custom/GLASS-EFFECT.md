...
**次要原因**：浮层填充度早期设为 `.72` 略显发实，后调至 `.56` 依然偏厚。
为达到用户明确要求的「背后的字能隐约看到一点点、但是高斯模糊看不清楚」，
最终将浅色浮层填充直接对齐至 **t3 黄金通透档位** `--dsh-glass-popover-fill: rgba(255, 255, 255, 0.38)`（深色 `.82`）：
既能清晰辨认选项，又能极具透光感地透出底层内容轮廓，并在 `blur(92px)` 强高斯模糊下彻底化开、无法辨认具体字形。
