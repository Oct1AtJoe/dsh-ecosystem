...
**液态主题（sequoia）的解法**：`bg-base` 取顶栏同色不透明值 `rgb(245,245,247)`、
`bg-app-image` 置 `none`、上述四处叠加全部清零，同时 **`backdrop-filter` 与半透明填充一律保留**
（去的是色差，不是玻璃）。侧边栏仍 `rgba(245,245,247,0.48)`，因底衬同色，alpha 合成后数值一致。

> **门禁**：`tests/sequoia-neutral-check.mjs` 锁死三条不变式（彩色清零 / 接缝叠加清零 / 玻璃保留），
