...
export function normalizeSidebarFont(value) {
    const n = typeof value === 'number' ? value : Number.parseInt(String(value ?? ''), 10);
    if (!Number.isFinite(n) || !Number.isInteger(n))
        return DEFAULT_SIDEBAR_FONT;
    if (n < SIDEBAR_FONT_MIN)
        return SIDEBAR_FONT_MIN;
    if (n > SIDEBAR_FONT_MAX)
        return SIDEBAR_FONT_MAX;
    return n;
}
/** 读取已保存的侧边栏字号（无记录或不可用时回退默认）。 */
export function readSidebarFont() {
...
/** 持久化侧边栏字号。 */
export function writeSidebarFont(px) {
    if (typeof localStorage === 'undefined')
        return;
    try {
 ...
 */
export function applySidebarFont(px) {
    if (typeof document === 'undefined')
        return;
    const value = `${normalizeSidebarFont(px)}px`;
    document.documentElement.style.setProperty(SIDEBAR_FONT_VARIABLE, value);
    if (document.body)
        document.body.style.setProperty(SIDEBAR_FONT_VARIABLE, value);
}
//# sourceMappingURL=sidebar-font.js.map