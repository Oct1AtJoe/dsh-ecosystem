...
import { optionalStringArray } from './build-preset/manifest.ts'
import { PLATFORM_MODULES, PRELOADED_CLIENT_EXTERNALS } from './build-preset/platform.ts'
import { clientBuildEnvironmentDefines } from './build-preset/client-build-environment.ts'
...
// Extracted-ecosystem layout: this preset now lives at <ecosystem>/plugins/,
// so one level up is the ecosystem root that owns packages/*/* and scripts/.
...
function workspaceManifest(id: string): WorkspaceManifest {
  ...
  // Monorepo layouts host packages under packages/*/* (upstream) or, in the
  // local plugin workspace, directly under plugins/*. Probe both so the same
  // preset serves either tree.
  for (const pattern of ['packages/*/*/package.json', 'plugins/*/package.json'] as const) {
  ...
  throw new Error(`tsdown: no packages/*/* or plugins/* package.json declares the name ${id}`)
}
